...
- 1v1 快速匹配池按知名度档位分组并先到先配，支持实时人数、排队位置和主动取消；匹配房固定使用系统默认判定标准
...
- 对战记录持久化与排行榜：客户端身份由可选「特征码」（≥4 位，跨设备同步）派生，未设置时退回匿名 ID；比赛结束后服务端把战绩写入数据库（`match_records/match_players/match_rounds`），首页排行榜支持按全部/单人/1v1/多人筛选，局内仍显示昵称
